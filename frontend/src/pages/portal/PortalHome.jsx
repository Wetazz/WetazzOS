...
export default function PortalHome() {
  ...
  return (
    <div className="p-8">
      <div className="text-[#FF3B30] font-mono text-xs tracking-[0.4em] mb-2">// MY GARAGE</div>
      <h1 className="font-display text-4xl mb-8">Welcome back, {user?.first_name}</h1>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {tiles.map(t => (
          <Link key={t.k} to={t.to} className="border border-white/10 p-6 hover:border-[#FF3B30] transition-colors" data-testid={`portal-tile-${t.k.toLowerCase()}`}>
      ...
      <div className="mt-10 border border-white/10 p-6 flex items-center justify-between">
        ...
        <div className="flex gap-2"><Link to="/book" className="px-5 py-2 bg-[#FF3B30] hover:bg-[#FF5B52] uppercase text-sm font-bold">Book</Link><Link to="/quote" className="px-5 py-2 border border-white/20 uppercase text-sm font-bold">AI quote</Link></div>
      </div>
    </div>
  );
}
