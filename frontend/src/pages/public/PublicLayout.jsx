...
export default function PublicLayout() {
  ...
  useEffect(() => { api.get("/business").then(r => setBiz(r.data)).catch(() => {}); }, []);
    ...
    <div className="min-h-screen bg-black text-white">
      <header className="sticky top-0 z-50 bg-black/70 backdrop-blur-xl border-b border-white/10" data-testid="public-header">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3" data-testid="brand-logo">
            <img src="/wetazz-logo-web.png" alt="Wetazz" className="h-12 w-12 object-contain" />
            <div>
      ...
      <footer className="border-t border-white/10 mt-24">
        <div className="max-w-7xl mx-auto px-6 py-10 grid md:grid-cols-4 gap-8 text-sm text-zinc-400">
          <div><div className="font-display text-white text-lg mb-2"><span className="text-[#B5FF2E]">WE</span><span className="text-[#FF2E93]">TAZZ</span></div><div>Paint · Panel · Mechanical</div><div className="mt-2 font-mono text-xs">{biz?.address || "89 Maxwell St, Wellington NSW 2820"}</div></div>
          ...
          <div><div className="text-white uppercase text-xs mb-2 tracking-widest">Contact</div><div className="font-mono" data-testid="footer-phone">{biz?.phone || "+61 2 0000 0000"}</div><div data-testid="footer-email">{biz?.email || "Office@wetazz.com.au"}</div></div>
