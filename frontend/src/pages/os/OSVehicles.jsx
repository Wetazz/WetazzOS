  ...
  return (
    <div className="p-8">
      <div className="flex items-baseline justify-between mb-6">
        ...
        <input data-testid="vehicle-search" placeholder="Rego / VIN / Make..." value={q} onChange={e=>setQ(e.target.value)} className="bg-[#0f0f10] border border-white/10 px-3 py-2 text-sm outline-none focus:border-[#FF3B30]"/>
      </div>
      <div className="grid md:grid-cols-3 gap-3">
        {rows.map(v => (
          <div key={v.id} className="border border-white/10 p-4" data-testid={`vehicle-card-${v.id}`}>
            ...
            <div className="font-mono text-xs text-[#FF3B30] mt-1">{v.registration || "NO REGO"}</div>
