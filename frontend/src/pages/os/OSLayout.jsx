...
import { LayoutDashboard, Users, Car, Calendar, Wrench, FileText, CreditCard, Inbox, Star, UserCog, Sparkles, LogOut } from "lucide-react";
    ...
    <div className="min-h-screen flex bg-black text-white">
      <aside className="w-60 border-r border-white/10 bg-[#0a0a0a] flex flex-col">
        <Link to="/" className="flex items-center gap-2 p-4 border-b border-white/10">
          ...
          <div><div className="font-display leading-none">WETAZZ · OS</div><div className="text-[10px] tracking-[0.3em] text-zinc-500 uppercase">{user?.role}</div></div>
        </Link>
        <nav className="flex-1 py-3 space-y-0.5 overflow-y-auto">
          {NAV.map(n => (
            <NavLink key={n.to} to={n.to} end={n.end} data-testid={`os-nav-${n.label.toLowerCase().replace(/ /g,'-')}`}
              className={({isActive}) => `flex items-center gap-3 px-4 py-2 text-xs uppercase tracking-wider transition-colors ${isActive ? "bg-[#FF3B30]/10 text-[#FF3B30] border-l-2 border-[#FF3B30]" : "text-zinc-400 hover:text-white hover:bg-white/5 border-l-2 border-transparent"}`}>
