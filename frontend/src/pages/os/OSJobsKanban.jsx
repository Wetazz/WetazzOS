...
function Card({ job }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}
      className="border border-white/10 bg-[#141414] p-3 mb-2 cursor-grab active:cursor-grabbing hover:border-[#B5FF2E] transition-colors" data-testid={`kanban-card-${job.id}`}>
      <div className="font-mono text-xs text-[#B5FF2E]">{job.job_number}</div>
  ...
  return (
    <div className="w-64 shrink-0 border border-white/10 bg-black/40 flex flex-col" data-testid={`kanban-col-${status}`}>
      <div className="p-3 border-b border-white/10 flex items-baseline justify-between">
        <div className="text-xs uppercase tracking-widest text-[#B5FF2E]">{status.replace(/_/g,' ')}</div>
  ...
  return (
    <div className="p-6">
      <div className="flex items-baseline justify-between mb-4">
        ...
        <button data-testid="new-job-btn" onClick={()=>setShowCreate(true)} className="px-4 py-2 bg-[#B5FF2E] hover:bg-[#C8FF5A] uppercase text-sm font-bold">+ New job</button>
      </div>
      ...
      {showCreate && (
        <form onSubmit={createJob} className="border border-white/10 p-5 mb-6 grid md:grid-cols-6 gap-3" data-testid="new-job-form">
          ...
          <button data-testid="job-create-submit" className="bg-[#B5FF2E] hover:bg-[#C8FF5A] uppercase text-sm font-bold md:col-span-1">Create</button>
