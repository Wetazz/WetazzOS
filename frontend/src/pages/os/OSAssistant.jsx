...
export default function OSAssistant() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <h1 className="font-display text-4xl mb-6 flex items-center gap-2"><Sparkles className="text-[#B5FF2E]"/> Workshop Assistant</h1>
      <div className="border border-white/10 p-4 h-[420px] overflow-y-auto space-y-3" data-testid="ai-chat-log">
        ...
        {messages.map((m,i)=>(
          <div key={i} className={`p-3 border ${m.role==='user' ? "border-[#B5FF2E]/30 bg-[#B5FF2E]/5" : "border-white/10 bg-white/5"}`}>
      ...
      <div className="mt-3 flex gap-2">
        <input data-testid="ai-input" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="Ask WETAZZ AI..." className="flex-1 bg-[#0f0f10] border border-white/10 px-3 py-2 outline-none focus:border-[#B5FF2E]"/>
        <button data-testid="ai-send" onClick={send} disabled={loading} className="px-4 py-2 bg-[#B5FF2E] hover:bg-[#C8FF5A] uppercase text-sm font-bold disabled:opacity-50">Send</button>
      </div>
    </div>
  );
}
