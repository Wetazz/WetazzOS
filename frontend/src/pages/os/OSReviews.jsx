  ...
  return (
    <div className="p-8">
      ...
      <form onSubmit={send} className="border border-white/10 p-4 grid md:grid-cols-4 gap-2 mb-6" data-testid="review-req-form">
        ...
        <button data-testid="review-req-submit" className="bg-[#B5FF2E] text-black uppercase text-sm font-bold">Request review</button>
      </form>
        ...
        {rows.map(r => (
          <div key={r.id} className="border border-white/10 p-4 flex items-center justify-between" data-testid={`review-row-${r.id}`}>
            ...
            <div className="flex items-center gap-1">
              {r.rating ? Array.from({length: 5}).map((_,i)=><Star key={i} size={16} className={i<r.rating? "text-[#B5FF2E] fill-[#B5FF2E]":"text-zinc-700"} />) : <span className="text-xs text-zinc-500 uppercase">Pending</span>}
            </div>
          </div>
        ))}
      </div>
